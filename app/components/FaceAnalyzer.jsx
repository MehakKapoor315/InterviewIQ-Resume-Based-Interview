...
export default function FaceAnalyzer({ webcamRef, isInterviewing, onUpdate }) {
                ...
                intervalRef.current = setInterval(async () => {
                    ...
                    try {
                        ...
                        if (detection) {
                            ...
                            // If nose tip is far from eye midpoint horizontally = looking away
                            const horizontalDeviation = Math.abs(noseTip.x - eyeMidX) / box.width;
                            // If nose tip is too far below eyes vertically = looking down
                            const verticalDeviation = (noseTip.y - eyeMidY) / box.height;

                            // Strict thresholds
                            const isLooking = horizontalDeviation < 0.25 && verticalDeviation < 0.6;
                            if (isLooking) statsRef.current.lookingCount++;

                            // ── Posture Detection ──
                            // Face must be large enough (close to camera)
                            const faceSizeRatio = box.height / videoHeight;
                            // Face must be in upper portion of frame
                            const faceTopRatio = (box.y + box.height / 2) / videoHeight;
                            // Face must be roughly centered horizontally
                            const faceCenterX = (box.x + box.width / 2) / videoWidth;
                            const horizontalCenter = Math.abs(faceCenterX - 0.5);

                            // Strict posture check
                            // Balanced posture check
                            const isGoodPosture =
                                faceSizeRatio > 0.12 &&      // face size (more lenient)
                                faceTopRatio < 0.65 &&        // face vertical position (more lenient)
                                horizontalCenter < 0.4;       // face horizontal center (more lenient)
                            ...
                            if (onUpdate) onUpdate({
                                eyeContact: Math.round((statsRef.current.lookingCount / statsRef.current.totalCount) * 100),
                                posture: Math.round((statsRef.current.goodPostureCount / statsRef.current.totalCount) * 100),
                            });
                        }
    ...
    const getLabel = (score) => {
        ...
        return score >= 70 ? '✅ Good' : score >= 50 ? '⚠️ Fair' : '❌ Poor';
    };
    ...
    return (
        <div style={{
            ...
            {/* Warning if face not visible */}
            {status === '⚠️ Face not detected' && (
                <p style={{ margin: '0.5rem 0 0', fontSize: '11px', color: '#ef4444' }}>
                    ⚠️ Please keep your face visible and centered in the camera
                </p>
            )}
        </div>
    );
